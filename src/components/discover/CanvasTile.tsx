...
export function CanvasTile({ item, dimmed, onSelect }: Props) {
  return (
    <button
      ...
      style={{ width: item.width, height: item.height, left: item.x, top: item.y }}
      data-id={item.id}
      data-kind={item.kind}
      onClick={(e) => onSelect(item, e.currentTarget)}
    >
      {item.image ? (
        <img className="canvas-tile__media" src={item.image} alt={item.name} loading="lazy" draggable={false} />
      ) : (
